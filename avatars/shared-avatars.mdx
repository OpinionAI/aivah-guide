...
View and manage all your deployed avatar agents in one place. Track shared deployments, copy deployment links, and monitor which agents (Worker or presenter) and models power each experience.

![Shared Avatar Agents](/images/shared-avatar-agents.png)
*Deployed avatar agents showing a Product Presenter and Worker Agents with different deployment formats and model information*
...
- **Agent Details**: Associated agent type and LLM model
...
- **Agent Name**: Demo Worker, Support Worker, Product Presenter, etc.
- **LLM Model**: Selected model (e.g., GPT 4.1 mini, GPT Realtime Mini, Gemini 2.5 Flash)
...
### Product Presenter Agent
**Configuration shown in image:**
- **Avatar**: Sales1 (Professional business representative)
- **Agent Type**: Presenter Agent anchored to a single product deck
- **Model**: GPT Realtime Mini (or any other selected realtime-capable model)
...
- **Content Refresh**: Ensure Worker Agents stay current
- **Performance Optimization**: Remove underperforming deployments

## Common Use Cases

### Sales Team
- **Product Presenter Agent**: For client demos and product presentations
