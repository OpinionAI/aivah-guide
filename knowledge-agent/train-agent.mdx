...
## Training Your Worker Agent

Training your Worker Agent involves optimizing its performance through strategic content management, testing, and refinement. While Aivah automatically trains your agent on all added content, this guide covers advanced techniques to maximize effectiveness through better organization and continual improvement.

<Note>Realtime Interactive Presenter Agents rely on a single PDF and inherit their performance from that document. Use this guide when you are managing Worker Agents with multiple knowledge sources.</Note>
...
Ready to implement specific agent types? Learn about [Realtime Interactive Presenter Agents](/knowledge-agent/presentation-agent) or explore [Real-time API configuration](/knowledge-agent/realtime) for advanced use cases.
