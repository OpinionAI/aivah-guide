...
<Note>Presenter Agents rely on a single PDF and inherit their performance from that document. Use this guide when you are managing Worker Agents with multiple knowledge sources.</Note>
...
Ready to implement specific agent types? Learn about [Presenter Agents](/knowledge-agent/presentation-agent) or explore [Real-time API configuration](/knowledge-agent/realtime) for advanced use cases.
