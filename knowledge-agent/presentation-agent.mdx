...
## Creating and Using Presenter Agents

Presenter Agents transform a single PDF deck or playbook into an interactive experience. Instead of clicking through slides, audiences can ask questions, jump to chapters, and get tailored explanations in real time—while still enjoying the same realtime, multimodal, and integration features available to Worker Agents.
...
- **Full toolkit** – Access the same integrations, web search, calling, and media generation capabilities as Worker Agents.
...
<Steps>
  ...
  <Step title="Name and Configure">
    ...
    3. Real-Time API and multimodal understanding are enabled by default—leave them on to retain full capabilities.
  </Step>
...
<Note>Presenter Agents do not accept additional knowledge sources. For multi-document experiences, create a Worker Agent instead—the rest of the capabilities remain identical.</Note>
