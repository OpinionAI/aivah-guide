...
- **Presentation Mode**: Leave off to create a Worker Agent, or turn on for a Realtime Interactive Presenter Agent
- **Real-Time API**: Keep enabled for live conversational interactions across channels
- **Multi Modal**: Enable if your plan supports image/video processing for Worker Agents
...
<Note>Realtime Interactive Presenter Agents rely on a single PDF upload. Delete the existing file to replace their presentation deck.</Note>
